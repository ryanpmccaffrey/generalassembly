...
lesson.path <- '~/DSI-4/classwork/working/R' # set this to your working directory
setwd(lesson.path)
getwd()
...
lm.model = lm(Rate ~ Verbal + Math, data = scores)
summary(lm.model)

new.df <- data.frame(Verbal=c(1.2, 1.3, 1.4), Math=c(1.2, 1.3, 1.4))
predict(lm.model, new.df)


library(rpart)
fit <- rpart(Rate ~ Verbal + Math,
             data=scores)
